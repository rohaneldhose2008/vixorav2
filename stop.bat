...
echo ====================================================================
echo.
echo [*] Performing privacy wipe (deleting local photo files)...
.\backend\venv\Scripts\python.exe -c "import pathlib; [f.unlink() for f in pathlib.Path('backend/photos').iterdir() if f.is_file()] if pathlib.Path('backend/photos').exists() else None; [f.unlink() for f in pathlib.Path('backend/watch_folder').iterdir() if f.is_file() and f.suffix.lower() in ('.jpg','.jpeg','.png')] if pathlib.Path('backend/watch_folder').exists() else None" 2>nul
